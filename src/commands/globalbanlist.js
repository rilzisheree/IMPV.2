...
export async function execute(interaction) {
  ...
  if (!allowed) {
    return interaction.reply({
      embeds: [new EmbedBuilder().setColor(0xF5C400).setDescription('❌ You do not have permission to use `/globalbanlist`.')],
      ephemeral: true,
    });
  }
  ...
  if (bans.length === 0) {
    return interaction.editReply({
      embeds: [new EmbedBuilder().setColor(0xF5C400).setDescription('✅ No users are globally banned.')],
    });
  }
